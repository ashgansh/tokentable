...
const getReleasableAmountCallback = (contract, grants) => async (id) => {
  const [tokenAddress, granter, beneficiary, createdBlockNumber] = id.split('-')
...
const getAdminTokenAllowanceCallback = (contract, admins) => async (tokenAddress, account) => {
...
export const getVestingData = async (chainId, contractAddress) => {
  ...
  const getAdminTokenAllowance = getAdminTokenAllowanceCallback(contract, admins)
